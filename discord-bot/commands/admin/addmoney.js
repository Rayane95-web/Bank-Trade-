...
module.exports = {
  data: new SlashCommandBuilder().setName('addmoney').setDescription('[DEV] Add money to a user')
  ...
  async execute(interaction) {
    ...
    try {
      ...
      user.wallet += amount; user.totalEarned += amount;
      db.saveUser(user);
      logAdminAction(interaction.user, 'ADD_MONEY', target, { amount });
      return interaction.reply({ embeds: [successEmbed('Money Added', `Added **${formatNumber(amount)}** coins to <@${target.id}>.\nNew wallet: **${formatNumber(user.wallet)}**`)] });
